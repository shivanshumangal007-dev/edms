import { ENDPOINT_PER_PAGE, DEFAULT_CATEGORY, FetchEndpointsByIndex, LoadEndpointsIndex, LoadMetaData, LoadSegments, LoadAllTags, } from "./index.js";
// ─────────────────────────────────────────────
// STATE
// ─────────────────────────────────────────────
...
const renderEndpoints = async (endpointsList) => {
    ...
    const currentEndpoints = (endpointsList ?? visibleEndpointsIndex).slice(start, end);
    ...
    endpointData.forEach((endpoint) => {
        ...
        const shortAnnotation = words.length > 4 ? words.slice(0, 6).join(" ") + "..." : words.join(" ");
        ...
        row.append(eidCell, methodCell, pathCell, annotationCell, qpCell, tagsCell, statusCell);
...
const applyAllFilters = async () => {
    ...
    if (needsDetailMatch && filteredIndex.length > 0) {
        ...
        for (let i = 0; i < details.length; i++) {
            ...
            if (filteredEntry)
                refinedIndex.push(filteredEntry);
        }
...
const pagination = (metaData) => {
    totalPages = Math.ceil(metaData.endpoints ? metaData.endpoints / ENDPOINT_PER_PAGE : 0);
...
LoadMetaData().then((metaData) => {
    if (metaData) {
        ...
        if (totalTagsEl)
            totalTagsEl.innerText = String(metaData.tags);
        const totalSegmentsEl = document.getElementById("total_segments");
        if (totalSegmentsEl)
            totalSegmentsEl.innerText = String(metaData.segments);
